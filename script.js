(function() {
    var animation = {
      ...
      get period() {
        var dateFuture = new Date("2024-12-21T18:00:00+05:30");
      ...
      year: function(className) {
        ...
        var displayYear = "2k25";
        for (var i = 0; i <= String(displayYear).length - 1; i++) {
          var digit = animation.element(year, "div", "digit", String(displayYear).substr(i, 1));
      ...
      animate: function() {
        ...
        for (var i=0; i<=50-1; i++) {
          ...
          timeline
            ...
            .to(triangle, duration, {top: "80%", rotationZ: animation.range(180, 360) * direction, rotationX: animation.range(180, 360) * direction}, time)
      ...
      render: function() {
        ...
        // Add the "check back later :)" message
        var message = document.querySelector(".check-back-message");
        if (!message) {
          message = animation.element(animation.newYear, "div", "check-back-message", "check back at 6 pm <3");
        }
